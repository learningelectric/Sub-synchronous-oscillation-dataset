...
class Mamba2(nn.Module):
    def __init__(self, d_model: int,  # model dimension (D)
                 n_layer: int = 6,  # number of Mamba-2 layers in the language model
                 d_state: int = 128,  # state dimension (N)
                 d_conv: int = 4,  # convolution kernel size
                 expand: int = 2,  # expansion factor (E)
                 headdim: int = 64,  # head dimension (P)
...
class NdMamba2(BaseNdMamba2):
    def __init__(self, cin, cout, mamba_dim, num_classes=10, height=32, weight=32, **mamba2_args):
        super().__init__(cin, cout, mamba_dim, **mamba2_args)
        # 定义两个线性层的分类头
        self.classification_layer = nn.Sequential(
            nn.Linear(cout*height*weight, 64),
            nn.LayerNorm(64),
            nn.Linear(64, num_classes)
        ) # 分类层，输出为 num_classes
        
    def forward(self, x):
        size = x.shape[2:]  # 获取输入的空间尺寸
        x = torch.flatten(x, 2)  # 将输入展平为 [b, c, size] -> [b, c, size]

        # 在这里处理 l，进行补零到64的倍数
        l = x.shape[2]
        x = F.pad(x, (0, (64 - x.shape[2] % 64) % 64))  # Pad l 到 64 的倍数
        x = rearrange(x, 'b c l -> b l c')  # 转成 1D 信号 [b, l64, c64]

        x = self.fc_in(x)  # 调整通道数到 mamba_dim
        x1, h1 = self.mamba2_for(x)
        x2, h2 = self.mamba2_back(x.flip(1))
        x2 = x2.flip(1)
        x = x1 + x2
        x = self.fc_out(x)  # 调整通道数到 cout
        x = rearrange(x, 'b l c -> b c l')  # 转成 2D 图片 [b, l64, c64]
        x = x[:, :, :l]  # 截取原图大小
        x = torch.unflatten(x, 2, size)  # 展开到原始形状

        # 在这里对输出进行平展
        b, c, h, w = x.shape  # 获取当前的 batch, channel, height, width
        x = x.reshape(b, c * h * w)  # 使用 reshape 展平为 [b, c * h * w]

        logits = self.classification_layer(x)

        return logits  # 返回最终的 logits


if __name__ == '__main__':
    # 通用的多维度双向mamba2
    net_n = NdMamba2(1, 32, 64,5,20,100).cuda()

    # 多维度数据
    x1 = torch.randn(2, 1, 20, 100).cuda() # 1d
